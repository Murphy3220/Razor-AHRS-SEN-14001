  ...
  pushMatrix(); {
    ...
    if (magnetomIndex < NUM_MAGN_SAMPLES && serial.available() >= 36) {
      ...
      while (serial.available() >= 36) {
        ...
        skipBytes(serial, 12);
        // Read magn data
        magnetom[magnetomIndex][0] = Float.intBitsToFloat(serial.read() + (serial.read() << 8) + (serial.read() << 16) + (serial.read() << 24));  // x
        magnetom[magnetomIndex][1] = Float.intBitsToFloat(serial.read() + (serial.read() << 8) + (serial.read() << 16) + (serial.read() << 24));
        magnetom[magnetomIndex][2] = Float.intBitsToFloat(serial.read() + (serial.read() << 8) + (serial.read() << 16) + (serial.read() << 24));  // z
        // Skip gyro data
        skipBytes(serial, 12);
        serial.clear();
      }
...
public void outputCalibration() {
  ...
  */
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Magnetometer_calibration" };
    if (passedArgs != null) {
